...
if __name__ == "__main__":
    ...
    if args.model.startswith("VGG16"):
        T = VGG16(1000)
        path_T = '../new_checkpoint/7643_kd_enc_vgg.pth'
    elif args.model.startswith('IR152'):
        T = IR50_vib(1000)
        path_T = '../new_checkpoint/mid_ir152_2194.pth'
    elif args.model == "FaceNet64":
        T = FaceNet64_vib(1000)
        path_T = '../new_checkpoint/mid_FaceNet_7577.pth'
    ...
    for i in range(1):
        ...
        for idx in range(10):
            ...
            if args.dist_flag == True:
                acc, acc5, acc_list, acc5_list = dist_inversion(G, D, T, E, iden, itr=i, lr=2e-2, momentum=0.9, lamda=100, iter_times=600, clip_range=1, improved=args.improved_flag, num_seeds=1)
            else:
                acc, acc5, acc_list, acc5_list = inversion(G, D, T, E, iden, itr=i, lr=2e-2, momentum=0.9, lamda=100, iter_times=600, clip_range=1, improved=args.improved_flag, num_seeds=1)
