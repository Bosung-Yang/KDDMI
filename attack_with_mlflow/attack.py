...
def dist_inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=5, exp_name=' '):
    ...
    return acc, acc_5, acc_var, acc_var5, acc_list, acc5_list


def inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=1,exp_name = ' '):
    device = "cuda"
    num_classes = 1000
    save_img_dir = exp_name
    os.makedirs(save_img_dir, exist_ok=True)
    success_dir = './gmi_res_success'
    os.makedirs(success_dir, exist_ok=True)
    ...
    for random_seed in range(num_seeds):
        ...
        for i in range(bs):
            gt = iden[i].item()
            sample = G(z)[i]
            save_tensor_images(sample.detach(), os.path.join(save_img_dir, "attack_iden_{}_{}.png".format(gt, r_idx)))
...
def deactivate_inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=1,exp_name= ' '):
    
    device = "cuda"
    num_classes = 1000
    save_img_dir = exp_name
    os.makedirs(save_img_dir, exist_ok=True)
    success_dir = './dgmi_res_success'
    os.makedirs(success_dir, exist_ok=True)
    ...
    for random_seed in range(num_seeds):
        ...
        for i in range(iter_times):
            ...
            if (i+1) % 1 == 0:
                fake_img = G(z.detach())
                eval_prob = E.deactivate(fake_img)[1]
                eval_iden = torch.argmax(eval_prob, dim=1).view(-1)
                acc = iden.eq(eval_iden.long()).sum().item() * 1.0 / bs
                acc_list.append(acc)
        ...
        fake = G(z)
        score = T.deactivate(fake)[1]
        eval_prob = E.deactivate(fake)[1]
        eval_iden = torch.argmax(eval_prob, dim=1).view(-1)
